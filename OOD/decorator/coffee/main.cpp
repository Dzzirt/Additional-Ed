...
{
	{
		...
		auto latte = make_unique<CMilkshake>(PortionSize::Average);
		// добавляем корицы
				...
				make_unique<CIceCubes>(				// | под нею - кубики льда
					make_unique<CLemon>(			// | | еще ниже лимон
						make_unique<CCinnamon>(		// | | | слоем ниже - корица
							make_unique<CTea>(TeaType::Green)),	// | | |   в самом сердце - Латте
						2),							// | | 2 дольки лимона
	...
	{
		auto beverage =
			make_unique<CCapuccino>(PortionType::Double)	// Наливаем чашечку латте,
			...
			<< MakeCondiment<CCream>();						// посыпаем шоколадной крошкой
...

	{
		auto beverage =
			...
			<< MakeCondiment<CLiquor>(LiquorType::Chocolate);	// посыпаем кокосовой стружкой
